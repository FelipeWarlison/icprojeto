<?php
  if (isset($_POST["add-pessoas-submit"])) {
    ...
    if(empty($nomePaciente)){
      header("Location: ../dashboardAreaPesquisador.php?error=empty");
    } else {
      $sql = "SELECT nome from pessoas where nome=?";
      $stmt = mysqli_stmt_init($conn);
      if(!mysqli_stmt_prepare($stmt,$sql)){
        header("Location: ../dashboardAreaPesquisador.php?error=sqlerror");
        exit();
      } else {
        ...
        if ($resultCheck > 0){
          header("Location: ../dashboardAreaPesquisador.php?error=pacienteexiste");
          exit();
        } else {
          ...
          if(!mysqli_stmt_prepare($stmt,$sql)){
            ...
            mysqli_stmt_bind_param($stmt,"s", $nomePaciente);
            mysqli_stmt_execute($stmt);
              header("Location: ../dashboardAreaPesquisador.php?error=noAd");
              exit();
          }
        }
      }
